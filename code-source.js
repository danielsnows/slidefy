// code-source.js — FONTE ÚNICA DO CÓDIGO DO PLUGIN
// Edite apenas este arquivo. O build (node build-templates.js) gera code.js a partir daqui.
// Não edite code.js manualmente; ele é sobrescrito pelo build.
...
const GOOGLE_FONTS = [
  ...
  { family: "Raleway", style: "Medium" },
  { family: "Poppins", style: "Regular" },
  { family: "Poppins", style: "Bold" },
  { family: "Poppins", style: "SemiBold" },
  { family: "Poppins", style: "Medium" },
  { family: "Alexandria", style: "Black" },
  { family: "Krona One", style: "Regular" },
  { family: "Instrument Sans", style: "Regular" },
  { family: "Instrument Sans", style: "Medium" },
...
async function preloadGoogleFonts() {
  for (const font of GOOGLE_FONTS) {
    try {
      ...
      console.warn("⚠️ Não foi possível carregar " + font.family + " " + font.style, e);
    }
  }
}

// ========== TEMPLATES EMBUTIDOS (substituído pelo build) ==========
// __BUILD_TEMPLATES_START__
...
var CULTO_JOVEM_MAIN_POSITIONS = { 'Culto': { x: 70, y: 4 }, 'Jo': { x: 20, y: -15 }, 'VEM': { x: 189, y: 320 } };
var CULTO_JOVEM_DATE_POSITION = { x: 812, y: 274 };
var CULTO_JOVEM_JUVENTUDE_POSITIONS = [
  { x: 0, y: 0 }, { x: 336, y: 176 }, { x: 672, y: 352 }, { x: 1008, y: 528 },
  { x: 1344, y: 704 }, { x: 1680, y: 880 }, { x: 2016, y: 1057 }
];
var JUVENTUDE_ROTATION_DEG = -90;
...
function applyCultoJovemTypography(root) {
  ...
  if (mainFrame) {
    ...
    if (mainFrame.children) {
      ...
      var culto = null, jo = null, vemFill = null, vemStroke = null, imgHeart = null;
      for (var i = 0; i < mainFrame.children.length; i++) {
        var n = mainFrame.children[i];
        if (n.type === 'TEXT') {
          var c = n.characters ? n.characters.trim() : '';
          if (c === 'Culto') culto = n;
          else if (c === 'Jo') jo = n;
          else if (c === 'VEM') {
            if (n.strokes && n.strokes.length > 0) vemStroke = n;
            else vemFill = n;
          }
        } else if (n.type === 'RECTANGLE' && n.name === 'Image') imgHeart = n;
      }
      var order = [culto, jo, vemStroke, imgHeart, vemFill];
      for (var o = 0; o < order.length; o++) {
        if (order[o]) mainFrame.insertChild(0, order[o]);
      }
    }
  }
  ...
  if (container && container.children) {
    ...
    for (var k = 0; k < juventudeTexts.length && k < CULTO_JOVEM_JUVENTUDE_POSITIONS.length; k++) {
      ...
      t.x = pos.x;
      t.y = pos.y;
      t.rotation = JUVENTUDE_ROTATION_DEG;
    }
    container.x = 1930;
    container.y = -114;
  }
